...
<div class="col-6 col-xl-6 col-xl-12 pl-0 text-center">
    <h2 style="font-size:20px">Get All Inactive Prepaid Customers Between:</h2>
    <form class="main-form" action="{{url('Inactive_Prepaid_customers_reporting')}}" method="GET" enctype="multipart/form-data">
 ...
 <div align="Center" style="padding:0px">
    <div class="col-lg-6 py-3 wow fadeInUp text-center" >
        ...
        All Inactive Prepaid Clients From {{Carbon\Carbon::parse($dateS)->format('D, M j, Y')}}
                ...
                <div class="card border-left-primary shadow h-100 py-2">
                    <div class="card-body">
                        <div class="row no-gutters align-items-center">
                            <div class="col mr-2">
                                <div class="text-lg font-weight-bold text-info text-uppercase mb-1"> Total Inactive Prepaid Clients</div>
          ...
          <table border=1 align='center' style= "margin-bottom: 50px;">
			...
			@foreach($shared as $shared)
                <tr style="background-color: #e8dbca;" align="left">
                    ...
                    <td style="padding: 10px; color: black;">{{$shared->customer_id}}</td>
                    <td style="padding: 10px; color: black;">{{$shared->clients}}</td>
                    <td style="padding: 10px; color: black;">{{$shared->service_plan}}</td>
                    <td style="padding: 10px; color: black;">{{$shared->service_type}}</td>
                    <td style="padding: 10px; color: black;">{{$shared->created_at}}</td>
                    @if($shared->status=='Active')
                        <td style="padding: 10px; color: black;  background-color: #8febab">{{$shared->status}}</td>
                    @elseif($shared->status=='Inactive')
                        <td style="padding: 10px; color: black;  background-color: yellow">{{$shared->status}}</td>
                    @else
                        <td style="padding: 10px; color: black;  background-color: #fc6d6d ">{{$shared->status}}</td>
                    @endif
