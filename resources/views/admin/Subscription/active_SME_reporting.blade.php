...
<div class="col-6 col-xl-6 col-xl-12 pl-0 text-center">
    <h2 style="font-size:20px">Get All Active SME Customers Between:</h2>
    <form class="main-form" action="{{url('active_sme_customers_report')}}" method="GET" enctype="multipart/form-data">
 ...
 <div align="Center" style="padding:0px">
    <div class="col-lg-6 py-3 wow fadeInUp text-center" >
        ...
        All Active SME Clients From {{Carbon\Carbon::parse($dateS)->format('D, M j, Y')}}
    ...
    <div class="container" align="text-center" style="padding-top: 5px;">
                ...
                <div class="card border-left-primary shadow h-100 py-2">
                    <div class="card-body">
                        <div class="row no-gutters align-items-center">
                            <div class="col mr-2">
                                <div class="text-lg font-weight-bold text-info text-uppercase mb-1"> Total Active SME Clients</div>
        ...
        <div align="center" style="margin-left:15px;padding-top:30px">
          <table border=1 align='center' style= "margin-bottom: 50px;">
			...
			@foreach($active_SME_reporting as $SME_cust)
                <tr style="background-color: #e8dbca;" align="left">
                    <td>{{$loop->iteration}}</td>
                    <td style="padding: 10px; color: black;">{{$SME_cust->customer_id}}</td>
                    <td style="padding: 10px; color: black;">{{$SME_cust->clients}}</td>
                    <td style="padding: 10px; color: black;">{{$SME_cust->service_plan}}</td>
                    <td style="padding: 10px; color: black;">{{$SME_cust->service_type}}</td>
                    <td style="padding: 10px; color: black;">{{$SME_cust->created_at}}</td>
                    @if($SME_cust->status=='Active')
                        <td style="padding: 10px; color: black;  background-color: #8febab">{{$SME_cust->status}}</td>
                    @elseif($SME_cust->status=='Inactive')
                        <td style="padding: 10px; color: black;  background-color: yellow">{{$SME_cust->status}}</td>
                    @else
                        <td style="padding: 10px; color: black;  background-color: #fc6d6d ">{{$SME_cust->status}}</td>
                    @endif
