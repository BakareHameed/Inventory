 ...
 <div align="Center" style="padding:0px">
    <div class="col-lg-6 py-3 wow fadeInUp text-center" >
      <h1 class="text-center" style="text-align:center;font-size:2rem">
          All Installations Done As At {{Carbon\Carbon::parse($currentDate)->format('D, M j, Y')}} 
      </h1>
    </div>
</div>
        ...
        <div class="flex flex-col items-center justify-center w-screen  bg-gray-900 py-10">
            <div class="flex flex-col mt-6">
              <div class="-my-2 overflow-x-auto sm:-mx-6 lg:-mx-8">
                    ...
                    <table class="min-w-full text-sm text-gray-400" >
                      <thead class="bg-gray-800 text-xs uppercase font-medium">
                          <tr style="background-color:black;">
                            ...
                            <th style="padding:10px; font-size: 20px; color: white ;">Done By</th>
                      ...
                      <tbody class="bg-gray-800">
                          ...
                          @foreach($installations as $installation)
                              <tr style="background-color: white;">
                                  ...
                                  <td style="padding: 10px; color: black;">{{$installation->survey_id}}</td>
                                  <td style="padding: 10px; color: black;">{{$installation->clients}}</td>
                                  <td style="padding: 10px; color: black;">{{$installation->contact_person_name}}</td>
                                  <td style="padding: 10px; color: black;">{{$installation->date}}</td>
                                  <td style="padding: 10px; color: black;">{{$installation->service_plan}}</td>
                                  <td style="padding: 10px; color: black;">{{$installation->service_type}}</td>
                                  @if($installation->download_bandwidth == null)
                                    <td style="padding: 10px; color: black;">{{$installation->download_bandwidth}}</td>
                                  @else
                                     <td style="padding: 10px; color: black;">{{$installation->download_bandwidth}}{{$installation->unit}}</td> 
                                  @endif

                                  @if ($installation->third_assigned_engr !== null && $installation->second_assigned_engr !== null && $installation->first_assigned_engr !== null)
                                    <td style="padding: 10px; color: black;">{{$installation->third_assigned_engr}}</td>
                                  @elseif ($installation->third_assigned_engr == null && $installation->second_assigned_engr !== null && $installation->first_assigned_engr !== null )
                                    <td style="padding: 10px; color: black;">{{$installation->second_assigned_engr}}</td>
                                  @else
                                    <td style="padding: 10px; color: black;">{{$installation->first_assigned_engr}}</td>
                                  @endif
                                    <td style="padding: 10px; color: black;">{{$installation->engr_name}}</td>
                                  @if($installation->feasibility!=null)
                                    <td style="padding: 10px; color: black;">{{$installation->feasibility}}</td>
                                  @else
                                    <td style="padding: 10px; color: black;"><strong><em>No report yet</em></strong></td>
                                  @endif
                                    <td style="padding: 10px; color: black;">{{$installation->name}}</td>
                                  @if($installation->duration_hours === null)
                                    <td style="padding: 10px; color: black;">{{$installation->duration_hours}}</td>
                                  @else
                                    <td style="padding: 10px; color: black;">{{$installation->duration_hours}} Hours</td>
                                  @endif
                                  <td style="padding: 10px; color: black;">{{$installation->duration_human}}</td>
                                  <td><a class="btn btn-primary" href="{{url('delivery_survey_report',$installation->survey_id)}}">view</a></td>
                              </tr>
                          @endforeach
                      </tbody>
                    </table>
                ...
                {{ $installations->links('user.customPagination') }}
                <!-- <span class="text-yellow" style="color:blue">{{$installations->links()}}</span> -->
