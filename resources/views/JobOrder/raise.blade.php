...
<div class="container" align="left" >
    @if(Session::has('message'))
        <div class=" col-12 col-lg-6 py-3 alert alert-success" role="alert"><strong>Success:</strong>{{Session::get('message')}}</div>
    @endif
    <div class="row">
                        ...
                        <table class="min-w-full text-sm text-gray-400">
                            <thead class="bg-gray-800 text-xs uppercase font-medium">
                                <tr style="background-color:black;">
                                    ...
                                    <th style="padding:10px; font-size: 20px; color: white ;">Reviewed?</th>
                            ...
                            @foreach($newClients as $client)
                                <tr style="background-color: white;" align="left">
                                    ...
                                    @else
                                        <td style="padding: 5px; color: black;"><strong> Not yet Approved</strong></td>
                                    @endif
                                    <td style="padding: 5px; color: black;">
                                        @if($client->edited_by !=null)
                                            <br> <span class="mt-2" style="color: Black;">Yes-- By-<em style="color:rgb(17, 82, 52)">
                                            {{$client->edited_by}}<em></strong></span>
                                        @else
                                            <strong>No</strong>
                                            <button id="btn+{{ $client->id }}" onclick="reviewed('{{ $client->id }}');" class=" block w-full mt-1 rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 main-form" style="padding: 2px;margin-bottom:2; color:white;background-color:darkviolet">Change</button>
                                            <form class="main-form" action="{{url('reviewForm',['ID'=>$client->id])}}" method="POST" enctype="multipart/form-data">
                                                @csrf
                                                <input type="hidden" name="jobOrderID" value="{{ $client->id }}">
                                                <div class="col-6 col-sm-6 py-2 credit_status" style="display:none" id="deliveryReview{{$client->id}}">
                                                    ...
                                                    <button type="submit" class="btn btn-primary" style="background-color:#68edc5; padding:4%;color:black">Submit</button>
                                                </div>
                                            </form>
                                        @endif
                                    </td>
                                    @if($client->raised_by !=null)
                                        <td style="padding: 5px; color: black;"><strong> Raised By-<em style="color:blue">
                                            ...
                                            <a style="padding: 5px;margin-bottom:5px" class="btn btn-primary" href="#" data-bs-toggle="modal" data-bs-target="#View{{$client->id}}">Job Order</a><span>
                                            <a style="padding: 10px;margin-bottom:10px" class="btn btn-warning" href="{{url('print-job-order-form',['id'=>$client->id])}}">Print</a><span>
                                        </td>
                                    @else
                                        <td style="padding: 5px; color: black;">
                                            <a style="padding: 5px;margin-bottom:5px" class="btn btn-primary" href="#" data-bs-toggle="modal" data-bs-target="#Raise{{$client->id}}">Raise</a><span>
                                        </td>
                                    @endif
                                </tr>
                            @endforeach
    ...
    <script type="text/javascript">
        ...
        function addRow(id){
            var tr = '<tr>'+
                        ...
                        '<td><input required type="number" class="amount" onkeyup="findTotal(this,id)" name="amount[]" class="form-control amount"/></td>'+
...
<!-- Script for Automatic addition of amount -->
    <script>
        function findTotal(q,id){
...
<!-- End of Script for auto adding -->

<!-- Beginning of script for Edit Form -->
    <script type="text/javascript">
        function edit_form(client_id){
            console.log('edit' + client_id);
            var form = document.getElementById("edit"+client_id);
...
<!-- End of Script For Edit form -->

<!-- Beginning of Credit Status Form -->
    <script type="text/javascript">
        function reviewed(id) {
            var form = document.getElementById("deliveryReview"+id);
            if (form.style.display === 'none') {
                form.style.display = 'block';
            } else {
                form.style.display = 'none';
