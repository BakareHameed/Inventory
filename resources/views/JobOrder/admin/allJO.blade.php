...
<body>
    ...
    @include('JobOrder.admin.installation-report-view')
    @endforeach
                        ...
                        <table class="min-w-full text-sm text-gray-400">
                            <thead class="bg-gray-800 text-xs uppercase font-medium">
                                <tr style="background-color:black;">
                                    ...
                                    <th style="padding:10px; font-size: 20px; color: white ;">Credited?</th>
                            ...
                            <tbody class="bg-gray-800">
                                ...
                                <tr style="background-color: white;" align="left">
                                    ...
                                    @endif
                                    <td style="padding: 5px; color: black;">{{$client->credited}}
                                        <button id="btn+{{ $client->jobOrderID }}" onclick="credited('{{ $client->jobOrderID }}');" class=" block w-full mt-1 rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 main-form" style="padding: 2px;margin-bottom:2; color:white;background-color:darkviolet">Change</button>
                                        <form class="main-form" action="{{url('adminJO-credited-status',['jobOrderID'=>$client->jobOrderID])}}" method="POST" enctype="multipart/form-data">
                                            @csrf
                                            <input type="hidden" name="jobOrderID" value="{{ $client->jobOrderID }}">
                                            <div class="col-12 col-lg-6 py-2 credit_status" style="display:none" id="{{$client->jobOrderID}}">
                                                <select required name="credited">
                                                    <option value="">--- Change Status ---</option>
                                                    <option>Credited</option>
                                                    <option>Not yet credited</option>
                                                </select>
                                    ...
                                    <td style="padding: 5px; color: black;"><strong> Raised By-<em style="color:blue">
                                        ...
                                        @endif
                                        <div>
                                            <a style="padding: 5px;margin-bottom:5px" class="btn btn-info" href="#" data-bs-toggle="modal" data-bs-target="#Surveys{{$client->id}}">Survey</a>
                                            @if($client->status==='Approved')
                                                <span> <a style="padding: 5px;margin-bottom:5px" class="btn btn-primary" href="#" data-bs-toggle="modal" data-bs-target="#Report{{$client->id}}">Job Order</a><span>
                                                @if(!empty($client->SLA))
                                                    <span> <a style="padding: 5px;margin-bottom:5px" class="btn btn-primary" href="#" data-bs-toggle="modal" data-bs-target="#SLA{{$client->id}}">SLA</a><span>
                                                @endif
                                            @else
