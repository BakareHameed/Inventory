...
<body>
    ...
    <div class="col-6 col-xl-6 col-xl-12 pl-0 text-center">
        <span>
        <h2 style="font-size:20px">Get Active Corporate Customers Between:</h2>
        <form class="main-form" action="{{url('All_active_corporate_customers_reporting')}}" method="GET" enctype="multipart/form-data">
            @csrf
            <div class="container">
                <div class="row">
    ...
    <div align="Center" style="padding:0px">
        <div class="col-lg-6 py-3 wow fadeInUp text-center" >
            <h1 class="text-center" style="text-align:center;font-size:2rem">All Active Corporate Clients</h1>
        </div>
    </div>
    ...
    <div class="container" align="text-center" style="padding-top: 5px;">
                ...
                <div class="col-xl-3 col-xl-4  mb-4">
                    <div class="card border-left-primary shadow h-100 py-2">
                        <div class="card-body">
                            ...
                            <div class="col mr-2">
                            <div class="text-lg font-weight-bold text-info text-uppercase mb-1"> Total No. Active Corporate Clients</div>
      ...
      <div align="left" style="padding-right:30px;padding-top:30px">
		<table border=1 align='left'>
			...
			@foreach($corporate as $corporate_clients)
                <tr style="background-color: #e8dbca;" align="left">
                    <td>{{$loop->iteration}}</td>
                    <td style="padding: 5px; color: black;">{{$corporate_clients->customer_id}}</td>
                    <td style="padding: 5px; color: black;">{{$corporate_clients->clients}}</td>
                    <td style="padding: 5px; color: black;">{{$corporate_clients->contact_person_name}}</td>
                    <td style="padding: 5px; color: black;">{{$corporate_clients->customer_email}}</td>
                    <td style="padding: 5px; color: black;">{{$corporate_clients->phone}}</td>
                    <td style="padding: 5px; color: black;">{{$corporate_clients->address}}</td>
                    <td style="padding: 5px; color: black;">{{$corporate_clients->service_plan}}</td>
                    <td style="padding: 5px; color: black;">{{$corporate_clients->service_type}}</td>
                    @if($corporate_clients->status=='Active')
                    <td style="padding: 5px; color: black;  background-color: #8febab">{{$corporate_clients->status}}</td>
                    @elseif($corporate_clients->status=='Inactive')
                        <td style="padding: 5px; color: black;  background-color: yellow">{{$corporate_clients->status}}</td>
                    @else
                        <td style="padding: 5px; color: black;  background-color: #fc6d6d ">{{$corporate_clients->status}}</td>
                    @endif
                </tr>
            @endforeach
		</table>      	
