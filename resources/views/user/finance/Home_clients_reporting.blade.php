...
<html lang="en">
  ...
  <div class="col-2 col-xl-6 col-xl-12 pl-0 text-center">
                        ...
                        <h2  style="font-align:center;font-size:2rem">Home Clients Between {{$dateS}} and {{$dateE}}</h2>
 ...
 <!-- <div class="card border-left-primary shadow h-100 py-2">
    <div class="card-body">
      <div class="row no-gutters align-items-center">
        <div class="col mr-2">
          <div class="text-lg font-weight-bold text-info text-uppercase mb-1"> Total No. of Home Clients</div>
    ...
    <div class="card-body">
      ...
      <div align="left" style="padding-right:30px;padding-top:30px">
        ...
        <th style="padding:5px; font-size: 20px; color: white ;">Amount Paid</th> -->
			...
			@foreach($home_report as $home)

			<tr style="background-color: #e8dbca;" align="left">
            <td>{{$loop->iteration}}</td>
            <td style="padding: 5px; color: black;">{{$home->id}}</td>
            <td style="padding: 5px; color: black;">{{$home->customer_id}}</td>
				<td style="padding: 5px; color: black;">{{$home->clients}}</td>
        <td style="padding: 5px; color: black;">{{$home->contact_person_name}}</td>
        <td style="padding: 5px; color: black;">{{$home->customer_email}}</td>
			
        <td style="padding: 5px; color: black;">{{$home->phone}}</td>
 
        <td style="padding: 5px; color: black;">{{$home->address}}</td>
       
				<td style="padding: 5px; color: black;">{{$home->service_plan}}</td>
				<td style="padding: 5px; color: black;">{{$home->service_type}}</td>
      
        @if($home->status=='Active')
        <td style="padding: 5px; color: black;  background-color: #8febab">{{$home->status}}</td>
        @elseif($home->status=='Inactive')
        <td style="padding: 5px; color: black;  background-color: yellow">{{$home->status}}</td>
        @else
        <td style="padding: 5px; color: black;  background-color: #fc6d6d ">{{$home->status}}</td>
        @endif
        

        <td style="padding: 5px; color: black;">{{$home->created_at}}</td>
        <td style="padding: 5px; color: black;">{{$home->updated_at}}</td>

        <!-- <td style="padding: 5px; color: black;" >{{$home->status}}</td>
        <td style="padding: 5px; color: black;" >{{$home->amount_paid}}</td> -->

        <td style="padding: 5px; color: black;" ><span>₦{{number_format($home->amount_paid)}}</span>
       
        <a style="padding: 5px;margin-bottom:5px" class="btn btn-secondary" href="{{url('activation_payment',$home->id)}}">Amount paid</a>
        </td>

          <td style="padding: 5px; color: black;" >
           <a style="padding: 5px;margin-bottom:5px" class="btn btn-danger" href="{{url('customer_deactivation',$home->id)}}">Deactivate</a><span>
         </td>
