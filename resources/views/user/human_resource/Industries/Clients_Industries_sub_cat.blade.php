...
<body>
  ...
  <div align="Center" style="padding:0px">
    <div class="col-lg-6 py-3 wow fadeInUp text-center" >
       ...
       Customer For {{$Industry_name}} Industry
    </h1>
    </div>
</div>
...
<div style="margin-left:100px;" class="col-xl-3 col-md-6 mb-4">
  <div class="card border-left-primary shadow h-100 py-2">
    ...
    <div style="margin-left:600px;" >
        <h1 class="text-center" style="text-align:left;font-size:1.5rem;">Filter By Category</h1>

        <form class="main-form" style="" action="{{url('industry_cat_filter',$Industry_name)}}" method="GET" enctype="multipart/form-data">

              @csrf

              <select name="Industries_sub_cat" class="block  mt-1 rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 form-control" >
                ...
                <option> clients without category </option> 
      ...
      <div align="center" style="padding-left:10px;padding-top:10px;padding-bottom:30px">
		...
		<table border=1>
          ...
          <tr style="background-color:black;">
			...
			@foreach($Industries_filter as $Industries_filters)

			<tr style="background-color: #e8dbca;" align="left">
                <td>{{$loop->iteration}}</td>
                <td style="padding: 15px; color: black;">{{$Industries_filters->clients}}</td>
                <td style="padding: 15px; color: black;">{{$Industries_filters->service_plan}}</td>
                <td style="padding: 15px; color: black;"> {{$Industries_filters->service_type}}</td>
                <td style="padding: 15px; color: black;">
                {{$Industries_filters->Industries_sub_cat}}
        
                  <div class="col-12 col-sm-6 py-2 wow fadeInRight" >
                     <a style="padding: 5px;margin-bottom:5px" class="btn btn-secondary" href="{{url('industry_change',$Industries_filters->id)}}">
                     Change
                     </a>
                  </div>
                ...
                <td style="padding: 15px; color: black;"> {{$Industries_filters->download_bandwidth}} {{$Industries_filters->unit}}</td>
                <td style="padding: 15px; color: black;"> {{$Industries_filters->created_at}}</td>
                @if($Industries_filters->status=='Active')
                <td style="padding: 5px; color: black;  background-color: #8febab">{{$Industries_filters->status}}</td>
                @elseif($Industries_filters->status=='Inactive')
                <td style="padding: 5px; color: black;  background-color: yellow">{{$Industries_filters->status}}</td>
                @else
                <td style="padding: 5px; color: black;  background-color: #fc6d6d ">{{$Industries_filters->status}}</td>
                @endif
            </tr>
